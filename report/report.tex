...
\begin{description}
    \item[Baseline~A (Classical)] COLREGs give-way, no weather-aware slowdown, unmanaged fatigue.
    \item[Baseline~B (Shore Rest Alerts)] Weather-aware speed reduction and slower fatigue build-up; no further mitigation.
    \item[Proposed System] Smart watch scheduling (lowest fatigue build-rate) on top of Baseline~B.
\end{description}

\noindent Storm-zone radio loss is shared by all methods (environmental, not a method privilege). We test:
...
Routes come from the offline network of Section~\ref{sec:routegen} (\texttt{data/ais\_paths.json}); ports are polygons in \texttt{data/ports.json}. Entering a port docks the ship.
...
\end{equation}
\vspace{0.6em}
\noindent where $q(\mathbf{p})$ is nominal link reliability, reduced to the storm-comms rate when $\mathbf{p}$ is in the storm zone ($0.08$ in Storm Corridor). A uniform draw below $P_{\text{comms}}$ executes the manoeuvre; otherwise geometry alone decides the encounter.
...
\begin{equation}
  ...
  \label{eq:overboard}
\end{equation}
drawn as independent Bernoulli trials. If $S_i < S^{\dagger} = 0.50$, the struck ship founders and the whole crew enters the liferaft (\textsc{Evac}, Section~\ref{sec:sar}). If it stays afloat, only the overboard draw enters the man-overboard chain (Section~\ref{sec:mob}).
...
Table~\ref{tab:kpis} defines the six hypothesis KPIs. Ship-hours add $0.25$\,h per \textsc{Active} ship per tick. A KPI \emph{fatality} is an unrecovered person-in-water within $\tau_{\text{mob}}$ (Section~\ref{sec:mob}); liferaft \textsc{Lost} is logged separately and does not enter KPI~1.
...
Confirmation criteria are those of Section~\ref{sec:stats}. Figure~\ref{fig:kpi-box} shows per-run KPI distributions; Figure~\ref{fig:hyp-delta} summarises effect sizes for the pre-registered contrasts.
