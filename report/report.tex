...
\begin{tikzpicture}[
  >=Stealth, font=\footnotesize,
  st/.style={draw, thick, rounded corners=3pt, minimum width=2.4cm,
             minimum height=0.62cm, align=center, inner sep=3pt},
  elab/.style={font=\scriptsize, inner sep=1pt},
  term/.style={draw, thick, circle, minimum size=0.28cm, inner sep=0pt},
]
  % Vertical happy path (labels sit beside edges --- never on them)
  ...
  \node[term, fill=white] (end) at (0,-6.9) {};

  \node[st, fill=red!15] (lost) at (4.2,-3.45) {\textsc{Lost}};

  \draw[->] (start) -- (mob) node[elab, midway, right=5pt]{dispatch};
  \draw[->] (mob) -- (tr) node[elab, midway, right=5pt]{delay $=0$};
  \draw[->] (tr) -- (se) node[elab, midway, right=5pt]{on scene};
  \draw[->] (se) -- (em) node[elab, midway, right=5pt]{CDP hit};
  \draw[->] (em) -- (ok) node[elab, midway, right=5pt]{board done};
  \draw[->] (ok) -- (end) node[elab, midway, right=5pt]{retire agent};

  % miss loop on the left of Searching
  \draw[->] ([yshift=1mm]se.west) .. controls +(-1.1,0.55) and +(-1.1,-0.55) ..
    node[elab, left=2pt]{miss} ([yshift=-1mm]se.west);

  % timeout branch to the right of Searching, then down to the shared sink
  \draw[->] (se.east) -- (lost.west)
    node[elab, midway, above=4pt]{liferaft $\tau$ elapsed};
  \draw[->] (lost.south) -- ++(0,-1.35) coordinate (to) -| (end);
  \node[elab, right=4pt] at ($(lost.south)!0.55!(to)$) {timeout};

  \node[align=left, font=\scriptsize, text=gray!40!black, anchor=north west]
    at (2.8,-6.35)
