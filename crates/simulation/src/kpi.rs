...
impl KpiAccumulator {
    ...
    pub fn record_collision_warning(&mut self, tta_ticks: u64) {
        self.tta_ticks_sum += tta_ticks as f64;
        self.tta_count += 1;
    }
    ...
    pub fn record_collision_outcome(
        ...
        let total_crew = (crew_i + crew_j) as u64;
        ...
        let p_evac = (BASE_EVAC_PROB
            + (1.0 - BASE_EVAC_PROB) * w.min(1.0)
            + 0.20 * max_fatigue)
            .min(1.0);
        if rng.gen::<f64>() < p_evac { self.evac_events += 1; }
        if rng.gen::<f64>() < p_evac { self.evac_events += 1; }
        ...
        let p_fatal = (BASE_FATAL_PROB_PER_CREW
            * (1.0 + 3.0 * w)
            * method_multiplier
            * fatigue_multiplier)
            .min(1.0);
